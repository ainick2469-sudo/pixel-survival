...
- High-distance scheduling now also runs behind a smoothed frame-time governor. At `192`, the governor squeezes chunk/far attach budgets, suppresses rear and lateral promotion first, and only recovers gradually after a spike instead of immediately reopening the whole outer detail ring.
...
- The smoke harness now proves `48`, `96`, and `192` startup directly and can run a scripted `192` forward-motion benchmark with JSONL telemetry, including the live governor percentage, so high-distance changes are validated under motion instead of only while standing still.
- Smoke validation now forces a windowed launcher path and accepts any window title that starts with `Pixel Survival`, which makes the benchmark harness more reliable than the earlier fullscreen-only startup path for automated captures.
